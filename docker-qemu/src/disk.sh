...
: "${DISK_SIZE:="16G"}"           # Size of the disk image
: "${DISK_TYPE:="virtio"}"        # Type of disk to emulate
: "${DISK_IO:="native"}"          # Type of disk I/O
: "${DISK_CACHE:="none"}"         # Type of disk cache
: "${DISK_DISCARD:="unmap"}"      # Type of disk trim
: "${DISK_FORMAT:="qcow2"}"       # Disk file format, can be: raw, qcow2, vmdk, vdi, vpc, vhdx
: "${DISK_FMT:="$DISK_FORMAT"}"   # For backwards compatibility
: "${DISK_FLAGS:=""}"             # Specifies the options for use with the qcow2 disk format
: "${DISK_ALLOC:="off"}"          # Allocate full disk size
